...
**Сумісність з авто.** У `Product` є опційне поле `fits` — масив id поколінь авто
(напр. `["jetta-5", "golf-6", "a4-b8"]`). Без цього поля товар вважається
універсальним. Дерево авто (марка → модель → покоління) і id — у
`frontend/src/config/vehicles.js`. Саме за `fits` працює фільтр «Виберіть авто»
(марка/модель/рік) і блок «Сумісність» на картці. У майбутньому власник під час
додавання товару має обирати авто/модель/рік — це й формує `fits`.
